...
from utils import user_requests
...
def main():
    ...
    user_requests.get_search_query_json_data(json_saver)
    # Удаляем из JSON вакансии без зарплаты
    user_requests.remove_muddy_vacancies(json_saver)
    ...
    top_vacancies_list = user_requests.show_top_vacancies_by_salary(vacancies_handler, all_vacancies_list)
    # Предлагаем отфильтровать топ вакансий и записываем результат в файл
    user_requests.filter_and_save_vacancies(vacancies_handler, top_vacancies_list)
